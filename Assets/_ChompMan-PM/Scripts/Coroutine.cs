...
{
    public Vector3[] targetScales; // Array de escalas 
    public Vector3[] targetPositions; // Array de posiciones 
    void Start()
    {
        StartCoroutine(Escala());
        StartCoroutine(CambiarPosicion());
        //StartCoroutine(AsignarColor()); Esperar hasta completar los campos de todos los elementos del array
    }
    ...
    {
        GameObject[] players = GameObject.FindGameObjectsWithTag("Player");
        yield return new WaitUntil(() => Time.frameCount % 3500 == 0);
